...
const MobileBottomNav = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-40 px-1 py-2 safe-area-pb">
      <div className="flex items-center justify-around w-full max-w-full mx-auto">
        {navigationItems.map((item) => {
          ...
          return (
            <Button
              ...
              className={`flex flex-col items-center px-1 sm:px-2 py-2 sm:py-3 h-auto min-w-0 flex-1 transition-colors text-center ${
                isActive
                  ? 'text-dna-forest bg-dna-mint/20'
                  : 'text-gray-600 hover:text-dna-forest'
              }`}
            >
              <item.icon className={`w-5 h-5 sm:w-6 sm:h-6 mb-1 ${isActive ? 'text-dna-forest' : ''}`} />
              <span className="text-xs sm:text-sm font-medium leading-none truncate">{item.title}</span>
            </Button>
          );
        })}
      </div>
