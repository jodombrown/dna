    ...
    <div className="relative flex min-h-0 flex-1 flex-col">
      <div ref={scrollRef} className="px-6 pb-8 space-y-6 overflow-y-auto">
        <Section label="Overview">
          <p className="text-body text-muted-foreground leading-relaxed">{entry.overview}</p>
        </Section>

        <Section label="What You Can Do">
          <ul className="space-y-2">
            {entry.whatYouCanDo.map((item, i) => (
              <li key={i} className="flex gap-2 text-body text-muted-foreground leading-relaxed">
                <span
        ...
        <Section label="Who It Is For">
          <p className="text-body text-muted-foreground leading-relaxed">{entry.whoItIsFor}</p>
        </Section>

        <Section label="How It Connects To The Other C's">
          <p className="text-body text-muted-foreground leading-relaxed">{entry.howItConnects}</p>
        </Section>
  ...
  const Header = entry ? (
    ...
    >
      {isMobile && (
        <div
          vaul-drawer-handle=""
          className="absolute left-1/2 top-2 h-1.5 w-handle-sm -translate-x-1/2 rounded-full bg-primary-foreground/50"
        />
      )}
  ...
  if (isMobile) {
    return (
      <Drawer open={!!entry} onOpenChange={(o) => !o && onOpenChange(null)}>
        <DrawerContent hideHandle className="flex max-h-sheet overflow-hidden rounded-t-dna-md p-0">
