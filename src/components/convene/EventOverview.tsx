...
import { useEventManagement } from '@/components/convene/management/EventManagementContext';
...
const EventOverview = () => {
  ...
  // event and isOrganizer come from EventDetail's context — the merged
  // page's only event fetch and only isOrganizer determination.
  const { event: contextEvent, isOrganizer } = useEventManagement();
  const event = contextEvent as unknown as Record<string, unknown>;
  ...
  useEffect(() => {
    if (rsvpError) console.error('[EventOverview] RSVP status query failed:', rsvpError);
    if (attendeesError) console.error('[EventOverview] Attendees query failed:', attendeesError);
    if (registrationCountError) console.error('[EventOverview] Registration count query failed:', registrationCountError);
