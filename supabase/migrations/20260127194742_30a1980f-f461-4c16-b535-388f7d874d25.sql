-- Fix discover_members to use exact matching for country text fields
...
BEGIN
  ...
  scored_profiles AS (
    ...
    WHERE
      ...
      -- FIXED: Use exact matching for text fields (prevents "Niger" matching "Nigeria")
      ...
      -- Array fields use && (overlaps) operator
      ...
      -- Search query uses ILIKE for fuzzy matching (appropriate for search)
      AND (
  ...
  ORDER BY
    CASE WHEN p_sort_by = 'match' THEN sp.calc_match_score ELSE 0 END DESC,
    CASE WHEN p_sort_by = 'recent' THEN sp.created_at ELSE '1970-01-01'::timestamptz END DESC,
    CASE WHEN p_sort_by = 'completion' THEN sp.profile_comp ELSE 0 END DESC,
    CASE WHEN p_sort_by = 'alphabetical' THEN 1 ELSE 0 END ASC,
    sp.full_name ASC
  LIMIT p_limit
  OFFSET p_offset;
END;
...
GRANT EXECUTE ON FUNCTION public.discover_members(uuid, text[], text[], text[], text, text, text[], text, text, integer, integer) TO authenticated;