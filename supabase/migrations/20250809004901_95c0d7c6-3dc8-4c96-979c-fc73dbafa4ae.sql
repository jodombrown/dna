-- Retry Migration: Drop existing conflicting functions, then (re)create RPCs and drop duplicate indexes

-- 0) Drop functions that may exist with incompatible signatures
DROP FUNCTION IF EXISTS public.rpc_dashboard_counts();
DROP FUNCTION IF EXISTS public.rpc_adin_recommendations_people(uuid, integer);
DROP FUNCTION IF EXISTS public.rpc_adin_recommendations_opportunities(uuid, integer, integer);
