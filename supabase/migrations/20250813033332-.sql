-- 1) Tighten RLS on public.profiles and remove overly permissive public reads
-- Safely (idempotently) enable RLS and replace SELECT policies
...
-- Create minimal, principle-of-least-privilege policies
DO $$
BEGIN
  ...
  -- INSERT: owner can insert own row (for apps that upsert on first save)
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy
...
-- 2) Public-safe accessors via RPC (SECURITY DEFINER) exposing only non-sensitive fields
-- List public profiles with safe fields and simple filters
...
-- Public-safe by-id accessor (no email/phone or private fields)
