-- Fix rpc_get_profile_bundle to include events where user is organizer (not just attendee)
...
BEGIN
  ...
  -- FIX: Get events where user is EITHER organizer OR attendee
  ...
  v_result := json_build_object(
    ...
    'tags', json_build_object(
      ...
      'focus_areas', COALESCE(v_profile.focus_areas, '[]'::jsonb),
      'impact_areas', COALESCE(v_profile.impact_areas, '[]'::jsonb),
      'available_for', COALESCE(v_profile.available_for, '[]'::jsonb),
      'industries', COALESCE(v_profile.industries, '[]'::jsonb),
      'regional_expertise', COALESCE(v_profile.regional_expertise, '[]'::jsonb),
      'professional_sectors', COALESCE(v_profile.professional_sectors, '[]'::jsonb)
    ),
    'activity', v_activity,
