-- Drop all versions of discover_members first
DROP FUNCTION IF EXISTS public.discover_members(uuid, text[], text[], text[], text, text, text, text, integer, integer, text[]);
DROP FUNCTION IF EXISTS public.discover_members(uuid, text[], text[], text[], text, text, text[], text, text, integer, integer);

-- Recreate discover_members without the 40% profile completion filter
...
BEGIN
  ...
  scored_profiles AS (
    ...
    WHERE
      ...
      -- NO profile completion filter - show ALL public profiles
