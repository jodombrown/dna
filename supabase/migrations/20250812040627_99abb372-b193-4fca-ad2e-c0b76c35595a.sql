-- 3-5) ADIN Connections Schema, RLS, and RPCs
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'profiles_adin_mode_check'
  ) THEN
    ALTER TABLE public.profiles
      ADD CONSTRAINT profiles_adin_mode_check
      CHECK (adin_mode IS NULL OR adin_mode IN ('quiet','standard','builder'));
  END IF;
END $$;

-- Connections additive columns and constraints
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'connections_status_check'
  ) THEN
    ALTER TABLE public.connections
      ADD CONSTRAINT connections_status_check
      CHECK (status IS NULL OR status IN ('requested','accepted','muted','archived','blocked'));
  END IF;
END $$;

-- connection_intentions constraints
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'connection_intentions_type_check'
  ) THEN
    ALTER TABLE public.connection_intentions
      ADD CONSTRAINT connection_intentions_type_check
      CHECK (type IN ('get_acquainted','knowledge_exchange','mentor','partner_explore','co-build','investor_interest','other'));
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'connection_intentions_visibility_check'
  ) THEN
    ALTER TABLE public.connection_intentions
      ADD CONSTRAINT connection_intentions_visibility_check
      CHECK (visibility IN ('shared','private'));
  END IF;
END $$;

-- connection_preferences columns and constraints
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'connection_preferences_cadence_check'
  ) THEN
    ALTER TABLE public.connection_preferences
      ADD CONSTRAINT connection_preferences_cadence_check
      CHECK (nudge_cadence IN ('off','low','medium','high','quiet','standard','builder'));
  END IF;
END $$;

-- connection_events columns and constraints
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'connection_events_type_check'
  ) THEN
    ALTER TABLE public.connection_events
      ADD CONSTRAINT connection_events_type_check
      CHECK (event_type IN ('message','share_resource','intro_made','attended_same_event','coauthored_post','task_completed','nudge_accepted','nudge_dismissed'));
  END IF;
END $$;

-- adin_recommendations constraints
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'adin_recommendations_type_check'
  ) THEN
    ALTER TABLE public.adin_recommendations
      ADD CONSTRAINT adin_recommendations_type_check
      CHECK (rec_type IN ('people','question','resource','event','microtask','collaborator'));
  END IF;
END $$;

-- adin_nudges constraints
ALTER TABLE public.adin_nudges ADD COLUMN IF NOT EXISTS payload jsonb;
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='adin_nudges' AND column_name='created_at') THEN
    ALTER TABLE public.adin_nudges ADD COLUMN created_at timestamptz DEFAULT now();
  END IF;
END $$;
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'adin_nudges_type_check'
  ) THEN
    ALTER TABLE public.adin_nudges
      ADD CONSTRAINT adin_nudges_type_check
      CHECK (nudge_type IN ('kickstart','checkin','suggest_event','suggest_resource','suggest_intro','microtask'));
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'adin_nudges_status_check'
  ) THEN
    ALTER TABLE public.adin_nudges
      ADD CONSTRAINT adin_nudges_status_check
      CHECK (status IN ('sent','accepted','dismissed','snoozed'));
  END IF;
END $$;

-- Optional: impact_attributions table
...
-- Helper: is_connection_participant(conn_id)
...
AS $fn$
  select exists (
    ...
    where c.id = p_connection and (c.a = auth.uid() or c.b = auth.uid())
  );
$fn$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='connections' AND policyname='connections_select_participants'
  ) THEN
    CREATE POLICY "connections_select_participants" ON public.connections FOR SELECT USING (public.is_connection_participant(id));
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='connections' AND policyname='connections_insert_participants'
  ) THEN
    CREATE POLICY "connections_insert_participants" ON public.connections FOR INSERT WITH CHECK ((a = auth.uid()) OR (b = auth.uid()));
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='connections' AND policyname='connections_update_participants'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='connection_intentions' AND policyname='ci_select_participants'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='connection_intentions' AND policyname='ci_insert_owner_participant'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='connection_intentions' AND policyname='ci_update_owner_participant'
  ) THEN
...
DO $$ BEGIN
  -- prefs policies already added earlier; ensure update policy exists
  IF NOT EXISTS (
    ...
    FOR UPDATE
    USING (user_id = auth.uid() AND public.is_connection_participant(connection_id))
    WITH CHECK (user_id = auth.uid() AND public.is_connection_participant(connection_id));
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='connection_events' AND policyname='ce_select_participants'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='connection_events' AND policyname='ce_insert_participants'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='adin_recommendations' AND policyname='ar_user_owned_select'
  ) THEN
    CREATE POLICY "ar_user_owned_select" ON public.adin_recommendations FOR SELECT USING (user_id = auth.uid());
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='adin_recommendations' AND policyname='ar_user_owned_insert'
  ) THEN
    CREATE POLICY "ar_user_owned_insert" ON public.adin_recommendations FOR INSERT WITH CHECK (user_id = auth.uid());
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='adin_recommendations' AND policyname='ar_user_owned_update'
  ) THEN
...
DO $$ BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='adin_nudges' AND policyname='Nudges update by owner'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='impact_attributions' AND policyname='ia_select_participants'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='impact_attributions' AND policyname='ia_insert_participants'
  ) THEN
...
DECLARE v_actor uuid := (select auth.uid()); v_a uuid; v_b uuid; BEGIN
  ...
  -- seed preferences for both participants if missing
  INSERT INTO public.connection_preferences(connection_id, user_id, nudge_cadence)
  VALUES (p_connection, v_a, 'standard')
  ON CONFLICT (connection_id, user_id) DO NOTHING;
  INSERT INTO public.connection_preferences(connection_id, user_id, nudge_cadence)
  VALUES (p_connection, v_b, 'standard')
  ON CONFLICT (connection_id, user_id) DO NOTHING;

  -- seed one kickstart nudge for each participant
  INSERT INTO public.adin_nudges(user_id, connection_id, nudge_type, message, status, created_at)
  VALUES (v_a, p_connection, 'kickstart', 'Start this connection with a hello.', 'sent', now());
  INSERT INTO public.adin_nudges(user_id, connection_id, nudge_type, message, status, created_at)
  VALUES (v_b, p_connection, 'kickstart', 'Start this connection with a hello.', 'sent', now());
END;
$fn$;
...
BEGIN
  ...
  INSERT INTO public.connection_intentions (
    id, connection_id, by_user, type, notes, visibility, created_at, target_outcome
  ) VALUES (
    v_id,
    p_connection,
    v_uid,
...
-- Update resolve_nudge to also log event
