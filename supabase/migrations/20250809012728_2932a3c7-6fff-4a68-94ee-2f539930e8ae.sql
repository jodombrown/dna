...
-- Only approved members of the space can read comments
create policy if not exists "read comments if space member"
  on public.task_comments for select using (
    public.is_member_of_space(space_id, (select auth.uid()), array['owner','admin','member'], true)
  );

-- Only space members can insert their own comments
create policy if not exists "insert own comment"
...
-- Author can delete own comments, owners/admins can delete any
create policy if not exists "delete comment by author or owner"
