-- DNA Onboarding Sprint: ensure schema + security invoker public_profile view + realtime + storage
...
-- 5) Public profile view: use SECURITY INVOKER (fix linter error)
DROP VIEW IF EXISTS public.public_profile;
CREATE VIEW public.public_profile SECURITY INVOKER AS
SELECT
  p.id,
  p.username,
  p.display_name,
...
-- Profiles: self write (avoid changing existing read policies)
DO $$
BEGIN
  IF NOT EXISTS (
