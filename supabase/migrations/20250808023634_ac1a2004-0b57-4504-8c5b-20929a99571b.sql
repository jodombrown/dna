-- Consolidate RLS policies to resolve linter warnings and finalize approval changes
...
-- Drop duplicate/old policies if present
DO $$
BEGIN
...
DO $$
BEGIN
  FOR policy_name IN SELECT policyname FROM pg_policies WHERE schemaname='public' AND tablename='verified_contributors'
  LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON public.verified_contributors', policy_name);
  END LOOP;
END $$;
