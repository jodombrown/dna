...
-- Ensure quick lookup and uniqueness using pg_constraint check
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'event_registrations_join_token_key'
  ) THEN
    ALTER TABLE public.event_registrations 
      ADD CONSTRAINT event_registrations_join_token_key UNIQUE (join_token);
  END IF;
END $$;
