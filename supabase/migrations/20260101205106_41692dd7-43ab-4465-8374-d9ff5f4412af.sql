-- Fix rpc_get_profile_bundle to include connection_status for proper Connect button behavior
...
BEGIN
  ...
  -- Possible values: 'none', 'pending_sent', 'pending_received', 'accepted', 'declined'
  v_connection_status := 'none';
  
  IF p_viewer_id IS NOT NULL AND NOT v_is_owner THEN
    -- Check if viewer sent a request to this profile
    SELECT 
      CASE 
    ...
    -- If no request found from viewer, check if profile owner sent a request to viewer
  ...
  -- Build and return the full bundle with connection_status for proper button behavior
  v_result := json_build_object(
    ...
    'permissions', json_build_object(
      ...
      'can_connect', NOT v_is_owner AND p_viewer_id IS NOT NULL AND v_connection_status = 'none'
    ),
