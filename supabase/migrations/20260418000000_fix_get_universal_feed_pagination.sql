-- Fix pagination in public.get_universal_feed.
-- The function already accepts p_offset, but NULL values from the client
-- (or an accidentally omitted parameter) would cause the OFFSET clause to
-- behave unexpectedly. Wrap p_offset with coalesce(p_offset, 0) so that the
-- React hook's paginated calls (p_offset = pageParam) work reliably.
-- Signature, return columns, and ORDER BY remain unchanged.
...
'Returns paginated universal feed for a viewer. Security: search_path set to public. Pagination uses coalesce(p_offset, 0).';
