-- Drop and recreate get_universal_feed with bookmark support
...
BEGIN
  ...
  SELECT
    ...
    COALESCE((SELECT COUNT(*) FROM post_comments WHERE post_id = p.id), 0)::BIGINT AS comment_count,
    COALESCE((SELECT COUNT(*) FROM post_shares WHERE post_id = p.id), 0)::BIGINT AS share_count,
    COALESCE(p.view_count, 0)::BIGINT AS view_count,
    COALESCE((SELECT COUNT(*) FROM post_bookmarks WHERE post_id = p.id), 0)::BIGINT AS bookmark_count,
      ...
      OR (
        ...
        AND EXISTS (
          ...
          WHERE (requester_id = p_viewer_id AND recipient_id = p.author_id AND status = 'accepted')
             OR (requester_id = p.author_id AND recipient_id = p_viewer_id AND status = 'accepted')
        )
      )
    )
    -- Tab filter
