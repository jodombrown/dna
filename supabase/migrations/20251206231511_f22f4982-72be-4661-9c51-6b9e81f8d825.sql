-- Fix rpc_get_profile_bundle to handle interest_tags as text[] and cast to jsonb
...
BEGIN
  ...
  -- Get events (via event_attendees, using organizer_id for host check)
  SELECT COALESCE(json_agg(json_build_object(
    'id', e.id,
    'title', e.title,
    'role', CASE 
  ...
  -- Note: interest_tags is text[], others are jsonb
