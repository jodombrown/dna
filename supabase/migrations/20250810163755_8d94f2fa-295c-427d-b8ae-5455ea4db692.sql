...
create policy if not exists ett_public_read on public.event_ticket_types for select using (true);
create policy if not exists ett_manage_owner_admin on public.event_ticket_types for insert with check (
  public.is_event_owner(event_id, (select auth.uid())) or public.is_admin_user((select auth.uid()))
);
create policy if not exists ett_update_owner_admin on public.event_ticket_types for update using (
  public.is_event_owner(event_id, (select auth.uid())) or public.is_admin_user((select auth.uid()))
) with check (
  public.is_event_owner(event_id, (select auth.uid())) or public.is_admin_user((select auth.uid()))
);
create policy if not exists ett_delete_owner_admin on public.event_ticket_types for delete using (
  public.is_event_owner(event_id, (select auth.uid())) or public.is_admin_user((select auth.uid()))
);
...
create policy if not exists erq_public_read on public.event_registration_questions for select using (true);
create policy if not exists erq_manage_owner_admin on public.event_registration_questions for insert with check (
  public.is_event_owner(event_id, (select auth.uid())) or public.is_admin_user((select auth.uid()))
);
create policy if not exists erq_update_owner_admin on public.event_registration_questions for update using (
  public.is_event_owner(event_id, (select auth.uid())) or public.is_admin_user((select auth.uid()))
) with check (
  public.is_event_owner(event_id, (select auth.uid())) or public.is_admin_user((select auth.uid()))
);
create policy if not exists erq_delete_owner_admin on public.event_registration_questions for delete using (
  public.is_event_owner(event_id, (select auth.uid())) or public.is_admin_user((select auth.uid()))
);
...
create policy if not exists ec_select_self_or_owner_admin on public.event_checkins for select using (
  exists (
...
create policy if not exists ec_insert_owner_admin on public.event_checkins for insert with check (
  exists (
    select 1 from public.event_registrations er
    join public.events e on e.id = er.event_id
...
create policy if not exists ec_update_owner_admin on public.event_checkins for update using (
  exists (
    select 1 from public.event_registrations er
    join public.events e on e.id = er.event_id
    where er.id = registration_id and (public.is_event_owner(e.id, (select auth.uid())) or public.is_admin_user((select auth.uid())))
...
create policy if not exists ec_delete_owner_admin on public.event_checkins for delete using (
  exists (
    select 1 from public.event_registrations er
    join public.events e on e.id = er.event_id
    where er.id = registration_id and (public.is_event_owner(e.id, (select auth.uid())) or public.is_admin_user((select auth.uid())))
...
create policy if not exists eb_owner_admin_read on public.event_blasts for select using (
...
create policy if not exists eb_owner_admin_insert on public.event_blasts for insert with check (
  exists (select 1 from public.events e where e.id = event_id and (public.is_event_owner(e.id, (select auth.uid())) or public.is_admin_user((select auth.uid()))))
);
create policy if not exists eb_owner_admin_update on public.event_blasts for update using (
  exists (select 1 from public.events e where e.id = event_id and (public.is_event_owner(e.id, (select auth.uid())) or public.is_admin_user((select auth.uid()))))
) with check (
  exists (select 1 from public.events e where e.id = event_id and (public.is_event_owner(e.id, (select auth.uid())) or public.is_admin_user((select auth.uid()))))
);
create policy if not exists eb_owner_admin_delete on public.event_blasts for delete using (
  exists (select 1 from public.events e where e.id = event_id and (public.is_event_owner(e.id, (select auth.uid())) or public.is_admin_user((select auth.uid()))))
);
...
create policy if not exists ea_public_insert on public.event_analytics for insert with check (true);
create policy if not exists ea_owner_admin_read on public.event_analytics for select using (
  exists (select 1 from public.events e where e.id = event_id and (public.is_event_owner(e.id, (select auth.uid())) or public.is_admin_user((select auth.uid()))))
);
