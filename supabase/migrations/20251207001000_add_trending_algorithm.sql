-- Migration: Add Trending/Top Feed Algorithm
-- Implements engagement scoring with time decay for the "Top" feed ranking mode
...
BEGIN
  ...
  ORDER BY
    -- Latest mode: Sort by creation time
    CASE WHEN p_ranking_mode = 'latest' THEN p.created_at END DESC,

    -- Top mode: Engagement score with time decay
    -- Formula: (likes*1 + comments*3 + shares*5 + bookmarks*2) / GREATEST(age_hours, 1)
    -- This gives higher scores to posts with more engagement that are also recent
    CASE WHEN p_ranking_mode = 'top' THEN
      (
        (COUNT(DISTINCT pl.id) * 1.0) +
        (COUNT(DISTINCT pc.id) * 3.0) +
        ((SELECT COUNT(*) FROM post_shares WHERE post_id = p.id) * 5.0) +
        ((SELECT COUNT(*) FROM post_bookmarks WHERE post_id = p.id) * 2.0)
      ) / GREATEST(EXTRACT(EPOCH FROM (NOW() - p.created_at)) / 3600, 1.0)
    END DESC NULLS LAST,
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION get_universal_feed TO authenticated, anon;

-- Add comment
COMMENT ON FUNCTION get_universal_feed IS 'Universal feed with trending algorithm: engagement score = (likes + comments*3 + shares*5 + bookmarks*2) / age_hours';
