-- Phase 3 Sprint 1: Engagement tracking and notification tables
...
ALTER TABLE public.notifications ADD CONSTRAINT notifications_type_check 
  CHECK (type IN ('reminder', 'announcement', 'invitation', 'system'));

-- Create indexes for performance
CREATE INDEX idx_user_engagement_tracking_user_event ON public.user_engagement_tracking(user_id, event_type, created_at);
CREATE INDEX idx_reminder_logs_user_type_scheduled ON public.reminder_logs(user_id, reminder_type, scheduled_at);
CREATE INDEX idx_notifications_user_type_delivered ON public.notifications(user_id, type, delivered_at);

-- Additional performance indexes
CREATE INDEX idx_user_engagement_tracking_cohort ON public.user_engagement_tracking(cohort, created_at);
CREATE INDEX idx_reminder_logs_status_scheduled ON public.reminder_logs(status, scheduled_at);
...
BEGIN
  INSERT INTO public.notifications (
    user_id, type, title, body, action_url, context, cohort
  ) VALUES (
    target_user_id, notification_type, notification_title, notification_body, 
    action_url_param, context_param, cohort_param
  ) RETURNING id INTO notification_id;
  
  -- Log the notification creation as an engagement event
  PERFORM public.log_engagement_event(
    target_user_id, 
    'notification_created', 
    jsonb_build_object('notification_id', notification_id, 'type', notification_type),
    cohort_param
  );
...
BEGIN
  ...
  WHERE id = notification_id 
    ...
    AND read_at IS NULL
  RETURNING user_id, type INTO target_user_id, notification_type;
  
  -- If notification was updated, log engagement event
