...
CREATE OR REPLACE FUNCTION public.calculate_profile_completeness_score_new(p_id UUID)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  -- Count filled text fields (using actual column names from the table)
  ...
  IF profile_record.profession IS NOT NULL AND profile_record.profession != '' THEN score := score + 1; END IF;
  IF profile_record.headline IS NOT NULL AND profile_record.headline != '' THEN score := score + 1; END IF;
  IF profile_record.current_location IS NOT NULL AND profile_record.current_location != '' THEN score := score + 1; END IF;
  IF profile_record.intro_text IS NOT NULL AND profile_record.intro_text != '' THEN score := score + 1; END IF;
...
CREATE OR REPLACE FUNCTION public.update_profile_completeness_trigger_new()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  NEW.profile_completeness_score := public.calculate_profile_completeness_score_new(NEW.id);
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER trigger_update_profile_completeness
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.update_profile_completeness_trigger_new();

-- Update existing profiles with calculated scores
UPDATE public.profiles 
SET profile_completeness_score = public.calculate_profile_completeness_score_new(id);