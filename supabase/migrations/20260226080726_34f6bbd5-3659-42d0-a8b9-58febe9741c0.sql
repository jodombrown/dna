...
CREATE OR REPLACE FUNCTION public.discover_members(
  ...
  p_ethnic_heritage text[] DEFAULT NULL
)
RETURNS TABLE (
  id uuid,
  full_name text,
...
BEGIN
  ...
  scored_profiles AS (
    ...
    WHERE
      ...
      -- Heritage filter: match against ethnic_heritage array, NOT regional_expertise
      ...
      AND (
        ...
        pr.country_of_origin ILIKE '%' || p_search_query || '%'
      )
  )
